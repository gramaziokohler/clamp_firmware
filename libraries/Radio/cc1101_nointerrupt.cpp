 ...
 *    False otherwise
 */
bool CC1101::sendDataSpecial(CCPACKET packet) {
    byte marcState;
    bool res = false;
    ...
    // Wait until TX FIF is empty
    tries = 0;
    while (tries++ < 1000 && (readStatusReg(CC1101_TXBYTES) & 0x7F) != 0) {
        // Check the remaining bytes
        //auto TXBYTES = (readStatusReg(CC1101_TXBYTES) & 0x7F);
    ...
    // If not, the transmission is a fail and the TX buffer needs to be flushed.
        setIdleState();       // Enter IDLE state
        flushTxFifo();        // Flush Tx FIFO
    }

    // Wait until radio leaves TX state
    while (tries++ < 1000 && ((marcState = readStatusReg(CC1101_MARCSTATE)) & 0x1F) == 0x13) {
        //Serial.print(F("Waiting to exit TX state (0x13)"));
    }


    //delayMicroseconds(1000);
    //auto state_1 = (readStatusReg(CC1101_MARCSTATE));
    //Serial.println("readStatusReg(CC1101_MARCSTATE)");
    //Serial.println(state_1);

    // It hink is not necessary
    //setIdleState();       // Enter IDLE state
    //flushTxFifo();        // Flush Tx FIFO
