/*
 Name:		Motor09_StoppingCondition.ino
 Created:	1/14/2020 7:06:04 PM
 Author:	leungp
*/
...
void perform_one_test(double kp, double ki, double kd, double velocityStepsPerSec, double accelStepsPerSecSq, double preDurationSec, double runDurationSec, double postDuration_Sec, double errorToStop) {
    ...
    while (micros() - testStartTimeMicros < total_duration_micros) {
        ...
        //Monitor if error is larger than threshold to stop
        double error = current_position_step - target_position_step;
        if (error < -errorToStop || error > errorToStop) break;
...
void loop() {
    if (Serial.available()) {
        ...
        double value_speed = str.toDouble();
        perform_one_test(0.040, 0.200, 0.0002, -1000, 3000, 0.1, 4.0, 0.0, 20);
        perform_one_test(0.040, 0.200, 0.0002, value_speed, 10000, 0.2, 20.0, 0.0, 100);
    }
}
